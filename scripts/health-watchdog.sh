...
HEALTH_OK=false
for i in 1 2 3; do
  if curl -sf --max-time 10 http://127.0.0.1/api/health > /dev/null 2>&1; then
    HEALTH_OK=true
    break
  fi
  sleep 5
...
FRONTEND_OK=false
for i in 1 2 3; do
  if curl -sf --max-time 10 -o /dev/null -w "%{http_code}" http://127.0.0.1:80 2>/dev/null | grep -qE "200|301"; then
    FRONTEND_OK=true
    break
  fi
  sleep 3
